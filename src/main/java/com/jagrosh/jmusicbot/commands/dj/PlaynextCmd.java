...
public class PlaynextCmd extends DJCommand
{
    ...
    public PlaynextCmd(Bot bot)
    {
        ...
        this.help = "воспроизводит следующий трэк";
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyWarning("Укажите ссылку на трэк или его название!");
            return;
        }
    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track)
        {
            ...
            {
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" Трэк (**"+track.getInfo().title+"**) длиннее разрешаемой длины трэка: `"
            ...
            String addMsg = FormatUtil.filter(event.getClient().getSuccess()+" Добавлен **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0?"для начала воиспроизведения":" позиция в очереди "+pos));
        ...
        {
            if(ytsearch)
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" Результатов по следующему запросу не найдено: `"+event.getArgs()+"`.")).queue();
            else
