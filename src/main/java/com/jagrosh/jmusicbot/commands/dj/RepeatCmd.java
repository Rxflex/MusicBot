...
public class RepeatCmd extends DJCommand
{
    public RepeatCmd(Bot bot)
    {
        super(bot);
        this.name = "repeat";
        this.help = "зацыкливает текущую очередь";
    ...
    protected void execute(CommandEvent event) 
    {
        ...
        {
            event.replyError("Валидные опции `on` или `off` (или оставьте пустым для включения/выключения)");
            return;
        }
        settings.setRepeatMode(value);
        event.replySuccess("Режим репит теперь `"+(value ? "ON" : "OFF")+"`");
    }
