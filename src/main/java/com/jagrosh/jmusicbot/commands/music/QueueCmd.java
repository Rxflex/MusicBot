...
public class QueueCmd extends MusicCommand 
{
    ...
    public QueueCmd(Bot bot)
    {
        super(bot);
        this.name = "queue";
        this.help = "отображает текущую очередь";
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        if(list.isEmpty())
        {
            ...
            Message built = new MessageBuilder()
                    .setContent(event.getClient().getWarning() + " Сейчас нет музыки в очереди!")
    ...
    private String getQueueTitle(AudioHandler ah, String success, int songslength, long total, boolean repeatmode)
    {
        ...
        return FormatUtil.filter(sb.append(success).append(" Текущая очередь | ").append(songslength)
                .append(" трэков | `").append(FormatUtil.formatTime(total)).append("` ")
